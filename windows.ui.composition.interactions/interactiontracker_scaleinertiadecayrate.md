...
The [ScaleInertiaDecayRate](interactiontracker_scaleinertiadecayrate.md) property defines the rate at which [InteractionTracker](interactiontracker.md) will slow to a stop when it has entered Inertia and scale is changing. The closer to 1, the faster [InteractionTracker](interactiontracker.md) will slow to a stop and vice versa. Unlike the [PositionInertiaDecayRate](interactiontracker_positioninertiadecayrate.md) which is defined as a Vector3, [ScaleInertiaDecayRate](interactiontracker_scaleinertiadecayrate.md) is defined as a single float.

## -property-value
Inertia decay rate for scale. Range is from 0 to 1.

## -remarks
Below is a graph of the equation that models the [ScaleInertiaDecayRate](interactiontracker_scaleinertiadecayrate.md) property. Note that as the value of the property approaches 1, the impact of inertia increases more significantly.
