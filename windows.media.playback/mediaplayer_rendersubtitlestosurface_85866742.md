...
## -remarks
Handle the [SubtitleFrameChanged](https://docs.microsoft.com/uwp/api/windows.media.playback.mediaplayer.SubtitleFrameChanged) event to receive a notification that the subtitle content has changed. Check the return value to see if subtitles were rendered. If the method returns false, then no subtitles were rendered. In this case you may decide to hide the subtitle render surface in your UI.
