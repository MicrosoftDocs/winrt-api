...
## -remarks
To improve scrolling performance, [ItemsWrapGrid](itemswrapgrid.md) creates and caches item containers for items that are off-screen on both sides of the viewport. The [CacheLength](itemswrapgrid_cachelength.md) property specifies the size of the buffers for the off-screen items. You specify [CacheLength](itemswrapgrid_cachelength.md) in multiples of the current viewport size. For example, if the [CacheLength](itemswrapgrid_cachelength.md) is 4.0, 2 viewports worth of items are buffered on each side of the viewport.
