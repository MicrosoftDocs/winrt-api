...
## -remarks
[SelectRange](listviewbase_selectrange.md) and [DeselectRange](listviewbase_deselectrange.md) provide a more efficient way to modify the selection than using the [SelectedItems](listviewbase_selecteditems.md) property. When you select items using index ranges, use the [SelectedRanges](listviewbase_selectedranges.md) property to get all selected ranges in the list.

When you call [SelectRange](listviewbase_selectrange.md), all items in the specified range are selected, regardless of their original selection state. You can select all items in a collection by using an [ItemIndexRange](../windows.ui.xaml.data/itemindexrange.md) with a [FirstIndex](../windows.ui.xaml.data/itemindexrange_firstindex.md) value of 0 and a [Length](../windows.ui.xaml.data/itemindexrange_length.md) value equal to the number of items in the collection.

> [!WARNING]
> Call the [SelectRange](listviewbase_selectrange.md) method only when the [SelectionMode](listviewbase_selectionmode.md) property is set to [Multiple](listviewselectionmode.md) or [Extended](listviewselectionmode.md). If you call [SelectRange](listviewbase_selectrange.md) when the [SelectionMode](listviewbase_selectionmode.md) is **Single** or **None**, an exception is thrown.
...
## -see-also
[SelectedRanges](listviewbase_selectedranges.md), [DeselectRange](listviewbase_deselectrange.md), [ItemIndexRange](../windows.ui.xaml.data/itemindexrange.md)