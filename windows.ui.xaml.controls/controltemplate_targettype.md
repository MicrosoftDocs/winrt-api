...
## -remarks
The typical way to specify a [TargetType](controltemplate_targettype.md) value is through a XAML attribute on the [ControlTemplate](controltemplate.md). When set in XAML, the meaning of [TargetType](controltemplate_targettype.md) and the values you provide take on some aspects of how types are represented in XAML. In particular, any prefixes that would be necessary to refer to a given type as a XAML element should also be included as the value of the [TargetType](controltemplate_targettype.md) value string. For example, if a template is intended to target a custom type that must be preceded by the already-mapped prefix "local" in a particular markup scope, then the [TargetType](controltemplate_targettype.md) value should include that same prefix. This behavior is enabled by built-in conversion behavior in the Windows Runtime XAML parser.
