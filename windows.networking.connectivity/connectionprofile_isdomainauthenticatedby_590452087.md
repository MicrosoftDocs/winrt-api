...
* Precedence: Only one mechanism reports true. If both LDAP (Active Directory) and a TLS-based endpoint would succeed, LDAP takes precedence and `IsDomainAuthenticatedBy(DomainAuthenticationKind.Ldap)` returns true while TLS returns false.
