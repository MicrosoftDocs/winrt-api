---
-api-id: T:Windows.Devices.Sms.SmsSendMessageResult
-api-type: winrt class
-api-device-family-note: xbox
---

<!-- Class syntax.
public class SmsSendMessageResult : Windows.Devices.Sms.ISmsSendMessageResult
-->

# Windows.Devices.Sms.SmsSendMessageResult

## -description
Encapsulates the results of calling [SmsDevice2.SendMessageAndGetResultAsync](smsdevice2_sendmessageandgetresultasync_123752871.md).

> [!NOTE]
> This functionality is only available to mobile operator apps and UWP apps given privileged access by mobile network operators, mobile broadband adapter IHV, or OEM. For more information, see [Mobile Broadband:  device apps](https://msdn.microsoft.com/library/windows/hardware/hh852368.aspx).

## -remarks
You access an [SmsSendMessageResult](smssendmessageresult.md) object through the asynchronous result of the [SmsDevice2.SendMessageAndGetResultAsync](smsdevice2_sendmessageandgetresultasync_123752871.md) method. For information about working with the results of asynchronous methods, see [Asynchronous programming (Windows Runtime apps)](https://msdn.microsoft.com/library/904ff97c-bb36-4ac5-9eda-a961e3639415).

## -examples

## -see-also


## -capabilities
cellularMessaging, sms
