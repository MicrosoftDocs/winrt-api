...
## -property-value
A string containing the [DeviceInformation](../windows.devices.enumeration/deviceinformation.md) ID of the SMS network interface (device). This value is typically used to retrieve details such as the sender and the body of the message. For example, you might use the ID to display a popup telling the user that a new message from a particular person has arrived. The combination of DeviceId and [MessageIndex](smsreceivedeventdetails_messageindex.md) is sufficient to retrieve the new message from the SMS device.
