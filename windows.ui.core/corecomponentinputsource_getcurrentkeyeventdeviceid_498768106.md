...
> [GetCurrentKeyEventDeviceId](corecomponentinputsource_getcurrentkeyeventdeviceid.md) is not supported for all input devices.
...
## -remarks
Use [GetCurrentKeyEventDeviceId](corecomponentinputsource_getcurrentkeyeventdeviceid.md) to differentiate between devices that can generate key events, such as multiple Xbox controllers. The device id is useful for associating the input device with a specific user account.
