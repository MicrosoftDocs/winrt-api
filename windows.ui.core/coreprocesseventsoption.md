...
Dispatch all events currently pending in the queue. If no events are pending, do not wait for a new event to be raised but instead return immediately. This option is provided specifically for view provider implementations with [IFrameworkView](../windows.applicationmodel.core/iframeworkview.md), and to be used in Windows Store app using DirectX with C++.
