...
The input data supplied to the [SignAsync](cryptographicengine_signasync.md) method is raw data that has not been hashed. To sign hashed data, use the [SignHashedDataAsync](cryptographicengine_signhasheddataasync.md) method.

If the key is a persisted key and the operation requires UI or takes a long time, use the [SignAsync](cryptographicengine_signasync.md) method instead of the [Sign](cryptographicengine_sign.md) method.
