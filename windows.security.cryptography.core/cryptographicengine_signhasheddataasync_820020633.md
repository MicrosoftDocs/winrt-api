...
The input data supplied to the [SignHashedDataAsync](cryptographicengine_signhasheddataasync.md) method is a hashed value. To sign raw data that has not been hashed, use the [SignAsync](cryptographicengine_signasync.md) method.

If the key is a persisted key and the operation requires UI or takes a long time, use the [SignHashedDataAsync](cryptographicengine_signhasheddataasync.md) method instead of the [SignHashedData](cryptographicengine_signhasheddata.md) method.
