...
## -remarks
Set [AllowAutoRedirect](httpbaseprotocolfilter_allowautoredirect.md) to **true** if you want the [HttpBaseProtocolFilter](httpbaseprotocolfilter.md) to automatically follow HTTP redirection headers to the new location of the resource. The maximum number of redirections to follow is set internally by the system.

If [AllowAutoRedirect](httpbaseprotocolfilter_allowautoredirect.md) is set to **false**, all HTTP responses with an HTTP status code from 300 to 399 are returned to whatever called the [HttpBaseProtocolFilter](httpbaseprotocolfilter.md), which might another filter that might then return them to the app.
