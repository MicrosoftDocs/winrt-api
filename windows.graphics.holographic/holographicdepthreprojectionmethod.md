...
Specifies the reprojection method used to process the depth buffer submitted for image stabilization for a particular holographic camera when a holographic frame is presented.
...
Each pixel will be reprojected using a smoothed and filtered depth value derived from depth information local to that pixel.
...
This value is only applicable when the app has also submitted a depth buffer for the same Holographic frame.
