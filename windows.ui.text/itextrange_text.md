...
## -remarks
The [Text](itextrange_text.md) property is the default property for [ITextRange](itextrange.md). As such, it is automatically invoked for the text range. For example: Some of the examples below use this fact. `print range`

The [Text](itextrange_text.md) method substitutes a string for the range text. For processing a single character, the [ITextRange.Character](itextrange_character.md) property is more efficient than the [Text](itextrange_text.md) property and doesn't require creating a single character range for storing a character.

If the range is degenerate, the [Text](itextrange_text.md) property lets you insert text easily. You can also delete the text in a range:`range.delete
range = ""`

You can use the [Text](itextrange_text.md) property to copy plain text from one place to another, simply by setting one range equal to another. The following statement sets the text of one range to that of another:`range1 = range2`
