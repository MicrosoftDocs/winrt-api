...
When programming with .NET, this structure is hidden and developers should use the [System.TimeSpan](/dotnet/api/system.timespan?redirectedfrom=MSDN) structure. The value can be **null**, because it's typed as `TimeSpan?` (a nullable [TimeSpan](/dotnet/api/system.timespan?redirectedfrom=MSDN)).
