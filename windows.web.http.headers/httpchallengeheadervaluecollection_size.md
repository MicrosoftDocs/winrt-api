...
## -remarks
The [Size](httpchallengeheadervaluecollection_size.md) value returns the true number of items. API that use the index, such as [IndexOf](httpchallengeheadervaluecollection_indexof.md), use a zero-based index, so you would subtract one from [Size](httpchallengeheadervaluecollection_size.md) in order to specify that you want the last item in the collection.
