...
## -examples
The function in the following example demonstrates how a cubic Bezier curve can be derived for each segment of an ink stroke (`stroke`) by using the [bezierCurveTo](https://msdn.microsoft.com/library/ff976007(v=vs.85).aspx) method. Each curve is then rendered through the 2-D context of the canvas (`ctx`).
