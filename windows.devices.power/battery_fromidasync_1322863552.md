...
## -remarks
[FromIdAsync](battery_fromidasync.md) will return null if the battery controller does not exist and the *deviceId* value is well-formed. If the *deviceId* value is not well-formed, such as "`AggregateBattery`," then the method will fail with `STATUS_OBJECT_NAME_INVALID`.
