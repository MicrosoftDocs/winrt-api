...
## -remarks
An advanced photo capture operation may require that multiple frames be captured and processed to produce the resulting image. This event notifies you when the source frames have been captured, which allows you to initiate another advanced photo capture as soon as the capture device is available, instead of waiting for the call to [CaptureAsync](advancedphotocapture_captureasync.md) to complete, which is raised only after the image processing of the captured frames is finished.
