...
(function () {
    ...
    // <Snippet1_JS>
    function getWiFiDirectDevices() {
        var deviceIds = new Array();
        try
        {
            var deviceSelector = Windows.Devices.WiFiDirect.WiFiDirectDevice.getDeviceSelector();
            Windows.Devices.Enumeration.DeviceInformation.findAllAsync(deviceSelector).then(
                function (devInfoCollection) {
                    for (i = 0; i < devInfoCollection.length; i++) {
                        deviceIds.push(devInfoCollection[i].name);
                    }
                    return deviceIds;
                });
        }
