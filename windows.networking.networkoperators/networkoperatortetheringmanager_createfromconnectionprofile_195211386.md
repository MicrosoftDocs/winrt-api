...
If the **wiFiControl** device capability is not set in the manifest, then calls to [CreateFromConnectionProfile](networkoperatortetheringmanager_createfromconnectionprofile.md) fail by throwing an exception.

Your code can determine whether the capability is set at runtime by calling [GetTetheringCapabilityFromConnectionProfile](networkoperatortetheringmanager_gettetheringcapabilityfromconnectionprofile.md). If **DisabledBySystemCapability** is returned, then the manifest does not set the required device capability. Note that even if **Enabled** is returned, your code should be written to handle errors and exceptions, since tethering capability can change at any time, outside your app's control or notice.
