...
## -remarks
A **DataCue** can be added to a [TimedMetadataTrack](timedmetadatatrack.md) with the [AddCue](timedmetadatatrack_addcue.md) method. When the cue's start time is reached, the [CueEntered](timedmetadatatrack_cueentered.md) event is raised and you can use the [MediaCueEventArgs](mediacueeventargs.md) passed into the handler to get a copy of the **DataCue**. The [Data](datacue_data.md) property of **DataCue** is an [IBuffer](../windows.storage.streams/ibuffer.md) that can contain any type of data. If you want to display text when the cue is encountered, the [TimedTextCue](timedtextcue.md) class is provided for this scenario.
