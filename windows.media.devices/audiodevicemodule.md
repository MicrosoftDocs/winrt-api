---
-api-id: T:Windows.Media.Devices.AudioDeviceModule
-api-type: winrt class
---

<!-- Class syntax.
public class AudioDeviceModule 
-->

# Windows.Media.Devices.AudioDeviceModule

## -description
Represents a single audio device module, which is a hardware effect processing unit or any other audio configuration module defined by an audio driver.

## -remarks
Get an instance of this class by calling [AudioDeviceModulesManager.FindAll](audiodevicemodulesmanager_findall_1149612203.md) or [AudioDeviceModulesManager.FindAllById](audiodevicemodulesmanager_findallbyid_1077023214.md).

## -see-also

## -examples

