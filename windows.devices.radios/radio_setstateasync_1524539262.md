...
If the state changes externally at the same time as the request, the operation can complete with a status indicating the final effective state rather than an intermediate step.
 ...
 A radio can be present but not user-controllable if required capabilities or policies are not in place; in that case its reported state is observable but attempts to change it have no effect.
