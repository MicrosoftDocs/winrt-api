...
This approach is more reliable than using [GetRadiosAsync](/uwp/api/windows.devices.radios.radio.GetRadiosAsync) in
situations where a USB radio has failed or been removed. When a radio fails or is removed,
[GetRadiosAsync](/uwp/api/windows.devices.radios.radio.GetRadiosAsync) returns no radio object.
[FindAllAsync](/uwp/api/windows.devices.enumeration.deviceinformation.FindAllAsync), however, still returns a valid
...
the radio object, which will report itself as 'Disabled'.

If the underlying hardware has been removed or is unavailable, the returned object reflects the last known
configuration and typically reports a Disabled state. A radio that no longer exists will not appear in new
