...
## -remarks
[UIElement](uielement.md) does not define any of the content model properties that [OnDisconnectVisualChildren](uielement_ondisconnectvisualchildren.md) might act upon. Such properties are introduced deeper in an inheritance, for example at the [ContentControl](../windows.ui.xaml.controls/contentcontrol.md) level.

[FrameworkElement](frameworkelement.md) uses an [OnDisconnectVisualChildren](uielement_ondisconnectvisualchildren.md) override to clear values from [DataContext](frameworkelement_datacontext.md) and [Tag](frameworkelement_tag.md). In general, if you override this method you should always call the base implementation so that the framework-intentional behavior that acts on [OnDisconnectVisualChildren](uielement_ondisconnectvisualchildren.md) can be preserved.
