...
Applications should avoid invoking code that puts a high load on the UI thread in response to [Resuming](application_resuming.md). Doing so gives appearance to the user that the shell is being unresponsive, when in fact it is your application code that is the throttle point. Delegate operations to other threads if possible, or use a staged or virtualized approach for rebuilding views and UI whenever the application is resumed.

## -examples

## -see-also
[Suspending](application_suspending.md), [OnActivated](application_onactivated.md), [Launching and resuming apps](XREF:TODO:nodepage.launching_and_resuming_apps__c__c___vb_)