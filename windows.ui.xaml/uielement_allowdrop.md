...
## -remarks
The value of [AllowDrop](uielement_allowdrop.md) determines whether various events related to being a drop target or responding to being dragged over can be handled. Such events only can be handled if [AllowDrop](uielement_allowdrop.md) is **true** on the [UIElement](uielement.md) that is a potential drop target. These events are:
+ [DragEnter](uielement_dragenter.md)
+ [DragLeave](uielement_dragleave.md)
+ [DragOver](uielement_dragover.md)
+ [Drop](uielement_drop.md)
 Each of the listed events is a routed event. If you want to handle a bubbling drag-drop event, the potential drop target must have [AllowDrop](uielement_allowdrop.md) set to **true**, and the object where the event is handled must have [AllowDrop](uielement_allowdrop.md) set to **true**. For more info on routed event concepts, see [Events and routed events overview](http://msdn.microsoft.com/library/34c219e8-3efb-45bc-8bbd-6fd937698832).
