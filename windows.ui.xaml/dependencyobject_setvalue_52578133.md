...
For app user-code, calling [SetValue](dependencyobject_setvalue.md) is not typically necessary. Usually, a Windows Runtime dependency property or a custom dependency property has a conventional property that wraps it, and you can just set the property value through a conventional dotted usage. Cases where you might still use [SetValue](dependencyobject_setvalue.md) are:
