...
## -description
Defines a repository for XAML resources, such as styles, that your app uses. You define the resources in XAML and can then retrieve them in XAML using the [{StaticResource} markup extension](https://msdn.microsoft.com/library/d50349b5-4588-4ebd-9458-75f629ccc395) and [{ThemeResource} markup extension](https://msdn.microsoft.com/library/8a1c79d2-9566-44aa-b8e1-cc7adad1bcc5) s. You can also access resources with code, but that is less common.
...
This XAML from file MainPage.xaml uses the [{StaticResource} markup extension](https://msdn.microsoft.com/library/d50349b5-4588-4ebd-9458-75f629ccc395) to access the **TitleStyle** and **H1Style** styles:
...
For info on how merged dictionary resources are resolved, see the "Merged resource dictionaries" section of [ResourceDictionary and XAML resource references](https://msdn.microsoft.com/library/e3cbfa3d-6af5-44e1-b9f9-c3d3ea8a25ce).

### The x:Key property

In XAML, the keys for [ResourceDictionary](resourcedictionary.md)items are declared by setting the [x:Key attribute](https://msdn.microsoft.com/library/141fc5af-80ee-4401-8a1b-17cb22c2277a)on elements that represent the XAML resources. Typically, if you try to put a child element that does not have a key value into a [ResourceDictionary](resourcedictionary.md), this throws a XAML parse exception or a Windows Runtimeexception. The exception condition might also be noted as a warning by XAML design surfaces. However, there are three notable cases where a [ResourceDictionary](resourcedictionary.md)child element won't require an [x:Key attribute](https://msdn.microsoft.com/library/141fc5af-80ee-4401-8a1b-17cb22c2277a)value:
+ A [Style](style.md) resource can use its [TargetType](style_targettype.md) value as the implicit resource key. For more info on how implicit keys for styles and control templates work, see [Styling controls](https://msdn.microsoft.com/library/ab469a46-faf5-42d0-9340-948d0edf4150).
+ The [ResourceDictionary](resourcedictionary.md) elements with [Source](resourcedictionary_source.md) values that represent [ResourceDictionary.MergedDictionaries](resourcedictionary_mergeddictionaries.md) values can't have an [x:Key attribute](https://msdn.microsoft.com/library/141fc5af-80ee-4401-8a1b-17cb22c2277a) on [ResourceDictionary](resourcedictionary.md). Within each merged dictionary file, (the one referenced by URI as its [Source](resourcedictionary_source.md)) you do need keys for each resource.
+ The [x:Name attribute](https://msdn.microsoft.com/library/4ff1f3ed-903a-4305-b2bd-dcd29e0c9e6d) can be used instead of [x:Key attribute](https://msdn.microsoft.com/library/141fc5af-80ee-4401-8a1b-17cb22c2277a), for legacy reasons. However, [x:Name attribute](https://msdn.microsoft.com/library/4ff1f3ed-903a-4305-b2bd-dcd29e0c9e6d) by itself doesn't enable XAML resource lookup of that item. The [x:Name attribute](https://msdn.microsoft.com/library/4ff1f3ed-903a-4305-b2bd-dcd29e0c9e6d) identifying convention is used for certain scenarios such as defining storyboarded animations. For more info, see [x:Name attribute](https://msdn.microsoft.com/library/4ff1f3ed-903a-4305-b2bd-dcd29e0c9e6d).
...
+ Strings or other constants that never need to be localized (strings and constants that do need to be localized shouldn't be in a [ResourceDictionary](resourcedictionary.md); for more info see [Quickstart: Translating UI resources](https://msdn.microsoft.com/library/6db5eb3b-7bb9-4776-a748-97743553d5e9))
...
For more info on how to use [ResourceDictionary](resourcedictionary.md) in code, see "Using a ResourceDictionary from code" section of [ResourceDictionary and XAML resource references](https://msdn.microsoft.com/library/e3cbfa3d-6af5-44e1-b9f9-c3d3ea8a25ce).
...
For more info on performance and XAML resource factoring, see [Optimize your XAML markup](https://msdn.microsoft.com/library/569e8c27-fa01-41d8-80b9-1e3e637d5b99).

## -examples

## -see-also
[ResourceDictionary and XAML resource references](https://msdn.microsoft.com/library/e3cbfa3d-6af5-44e1-b9f9-c3d3ea8a25ce), [Application resources and localization sample](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/ApplicationResources), [{ThemeResource} markup extension](https://msdn.microsoft.com/library/8a1c79d2-9566-44aa-b8e1-cc7adad1bcc5), [{StaticResource} markup extension](https://msdn.microsoft.com/library/d50349b5-4588-4ebd-9458-75f629ccc395), [x:Key attribute](https://msdn.microsoft.com/library/141fc5af-80ee-4401-8a1b-17cb22c2277a)
