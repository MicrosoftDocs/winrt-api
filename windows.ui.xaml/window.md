...
## -remarks
This class represents the window of the current [Application](application.md). In the same manner as the static [Application](application_application.md) property, the static [Current](application_current.md) property will return the app window object. From this object, an app can utilize the [Dispatcher](window_dispatcher.md) or determine the size of the [Window](window.md) from the [Bounds](window_bounds.md) property. The most common usage for [Window](window.md) is setting a [UIElement](uielement.md) to the [Content](window_content.md) that represents the app UI. This is usually done as part of app activation (for example in the [OnLaunched](application_onlaunched.md) override.) You can change this window content throughout the lifetime of the window.

There is no XAML representation of the [Window](window.md) class because it is not a control.

A [Window](window.md) object is just surfacing information from [CoreWindow](../windows.ui.core/corewindow.md), which in turn is referencing the window created by the system.
...
## -see-also
[OnLaunched](application_onlaunched.md), [Handle app activation](http://msdn.microsoft.com/library/da9a6a43-f09d-4512-a2ab-9b6132431007), [CoreWindow](../windows.ui.core/corewindow.md)