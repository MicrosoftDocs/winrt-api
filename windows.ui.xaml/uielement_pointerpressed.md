...
[!code-csharp[PointerMoved](../windows.ui.input.inking/code/PointerInput/csharp/MainPage.xaml.cs#SnippetPointerMoved)][!code-csharp[PointerMoved](../windows.ui.input.inking/code/PointerInput_UWP/csharp/MainPage.xaml.cs#SnippetPointerMoved)]
...
[PointerPressed](uielement_pointerpressed.md) is sometimes used as the initiating event when capturing the pointer, so that further pointer-related events are all captured by a particular element so long as the pointer remains down. To capture a pointer, you call the [CapturePointer](uielement_capturepointer.md) method on a specific UI element that should maintain the capture. This is usually done within a [PointerPressed](uielement_pointerpressed.md) event handler, and you call [CapturePointer](uielement_capturepointer.md) on the event's *sender*. For more info on how to capture a pointer and why you might want to do so, see [CapturePointer](uielement_capturepointer.md).
