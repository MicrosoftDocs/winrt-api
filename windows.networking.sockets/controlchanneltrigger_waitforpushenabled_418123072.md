...
## -remarks
Once an app is connected, it must call the [WaitForPushEnabled](controlchanneltrigger_waitforpushenabled.md) method in order to enable the system to complete the internal configuration of the control channel trigger. If an app tries to use the network trigger mechanism without calling the [WaitForPushEnabled](controlchanneltrigger_waitforpushenabled.md) method, it will get an exception.

This call to the [WaitForPushEnabled](controlchanneltrigger_waitforpushenabled.md) method is to be made only after the transport connection is established.
