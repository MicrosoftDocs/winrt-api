...
## -remarks
The [BindEndpointAsync](datagramsocket_bindendpointasync_1396029045.md) method binds the [DatagramSocket](datagramsocket.md) to the local hostname or IP address specified in the *localHostName* parameter and the local service name or UDP port number address specified in the *localServiceName* parameter. If the *localHostName* parameter is null, the system will select the local IP address on which to bind. If the *localServiceName* parameter is an empty string, the system will select the local UDP port on which to bind.

The [BindEndpointAsync](datagramsocket_bindendpointasync_1396029045.md) method will fail if another app using UDP (another [DatagramSocket](datagramsocket.md), for example) has already been bound to the local IP address and UDP port specified in the *localHostName* and *localServiceName* parameters.
