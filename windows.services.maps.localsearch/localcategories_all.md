---
-api-id: P:Windows.Services.Maps.LocalSearch.LocalCategories.All
-api-type: winrt property
---

<!-- Property syntax
public string All { get; }
-->

# Windows.Services.Maps.LocalSearch.LocalCategories.All

## -description
Gets a value that represents all search categories.

> [!NOTE]
> This API is not available to all Windows apps. Unless your developer account is specially provisioned by Microsoft, calls to this API will fail at runtime. For more information about the [Windows.Services.Maps.LocalSearch](windows_services_maps_localsearch.md) namespace, please work with your Microsoft Account Team representative.

## -property-value
A value that indicates search results are not limited by search categories.

## -remarks
If the [All](localcategories_all.md) category is specified, the [FindLocalLocationsAsync](locallocationfinder_findlocallocationsasync.md) method returns results for all search categories.

## -examples

## -see-also
