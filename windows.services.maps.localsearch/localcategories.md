---
-api-id: T:Windows.Services.Maps.LocalSearch.LocalCategories
-api-type: winrt class
---

<!-- Class syntax.
public class LocalCategories 
-->

# Windows.Services.Maps.LocalSearch.LocalCategories

## -description
Provides names of search categories that you can use to limit search results when calling the [FindLocalLocationsAsync](locallocationfinder_findlocallocationsasync.md) method.

> [!NOTE]
> This API is not available to all Windows apps. Unless your developer account is specially provisioned by Microsoft, calls to this API will fail at runtime. For more information about the [Windows.Services.Maps.LocalSearch](windows_services_maps_localsearch.md) namespace, please work with your Microsoft Account Team representative.

## -remarks

## -examples

## -see-also
