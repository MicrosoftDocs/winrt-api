...
A SortEntryVector is enumerable, so you can use language-specific syntax such as **foreach** in C# to enumerate the items in the collection. The compiler does the type-casting for you and you won't need to cast to `IEnumerable<SortEntry>` explicitly. If you do need to cast explicitly, for example if you want to call [GetEnumerator](/dotnet/api/system.collections.generic.ienumerable-1.getenumerator?redirectedfrom=MSDN#System_Collections_Generic_IEnumerable_1_GetEnumerator), cast to [IEnumerable&lt;T&gt;](/dotnet/api/system.collections.generic.ienumerable-1?redirectedfrom=MSDN) with a [SortEntry](sortentry.md) constraint.
