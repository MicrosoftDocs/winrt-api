...
## -description
Provides the peer's behavior when a Microsoft UI Automation client calls [GetFlowsTo](../windows.ui.xaml.automation/automationproperties_getflowsto.md) or an equivalent Microsoft UI Automation client API.

## -returns
A list of automation elements that suggests the reading order after the current automation element.

## -remarks
The "Core" methods are the standard implementations that perform the default action of an associated UI Automation client-callable method. You can override any of the "Core" methods to return alternative values in a custom automation peer. In this case, [GetFlowsToCore](automationpeer_getflowstocore_1266863981.md) is invoked any time that [GetFlowsTo](../windows.ui.xaml.automation/automationproperties_getflowsto.md) is called.
