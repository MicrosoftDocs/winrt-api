---
-api-id: P:Windows.Globalization.Language.CurrentInputMethodLanguageTag
-api-type: winrt property
---

<!-- Property syntax
public string CurrentInputMethodLanguageTag { get; }
-->

# Windows.Globalization.Language.CurrentInputMethodLanguageTag

## -description
Gets the [BCP-47](http://go.microsoft.com/fwlink/p/?linkid=227302) language tag for the currently enabled keyboard layout or Input Method Editor (IME).

## -property-value
The [BCP-47](http://go.microsoft.com/fwlink/p/?linkid=227302) language tag.

## -remarks


## -examples

## -see-also
[BCP-47 language tags](http://go.microsoft.com/fwlink/p/?linkid=227302)