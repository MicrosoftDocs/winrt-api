---
-api-id: T:Windows.UI.Text.Core.CoreTextServicesManager
-api-type: winrt class
---

<!-- Class syntax.
public class CoreTextServicesManager : Windows.UI.Text.Core.ICoreTextServicesManager
-->

# Windows.UI.Text.Core.CoreTextServicesManager

## -description
The entry point to all services provided by the Windows core text APIs and the text input server. This object is associated with an application's UI thread (the thread that [CoreWindow](../windows.ui.core/corewindow.md) runs on).

## -remarks
To obtain an object of this type, call [CoreTextServicesManager.GetForCurrentView](coretextservicesmanager_getforcurrentview.md).

## -examples

## -see-also
