...
## -remarks
If you try scale an image stored in an indexed pixel format using the [BitmapTransform](bitmaptransform.md) member, [FlushAsync](bitmapencoder_flushasync_491532439.md) fails with HRESULT [WINCODEC_ERR_INVALIDPARAMETER](https://docs.microsoft.com/windows/desktop/wic/-wic-codec-error-codes) . Instead, you must use [GetPixelDataAsync](/uwp/api/windows.graphics.imaging.bitmapdecoder.getpixeldataasync) to obtain the scaled pixel data and then use [SetPixelData](bitmapencoder_setpixeldata_644328650.md) to set it on the encoder.
