...
## -remarks
The [PrintTaskOptions](printtaskoptions.md) class provides access to the different values that define how the content in the print task is to be formatted during printing. The [PrintTaskOptions](printtaskoptions.md) object may contain information like the size of the printed page, its orientation, and the type of media on which the content will be printed. In addition the [PrintTaskOptions](printtaskoptions.md) object provides access to the list of options to be displayed in the print window.

While each option in [PrintTaskOptions](printtaskoptions.md), for example [MediaSize](printtaskoptions_mediasize.md) or [MediaType](printmediatype.md), has a specific set of supported values, all the options support the same general pattern for representing some common cases. The common case values are *Default*, *NotAvailable* and *PrinterCustom*.
