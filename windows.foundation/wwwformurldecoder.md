...
For .NET usage, WwwFormUrlDecoder has the projected APIs of a generic [IReadOnlyList](https://docs.microsoft.com/dotnet/api/system.collections.generic.ireadonlylist-1?redirectedfrom=MSDN) with a [IWwwFormUrlDecoderEntry](iwwwformurldecoderentry.md) constraint. The APIs that are available for each language are indicated in the member lists.
...
WwwFormUrlDecoder also has the projected APIs of a generic [IReadOnlyList](https://docs.microsoft.com/dotnet/api/system.collections.generic.ireadonlylist-1?redirectedfrom=MSDN) with a [IWwwFormUrlDecoderEntry](iwwwformurldecoderentry.md) constraint, but these APIs aren't commonly used.
