---
-api-id: M:Windows.Storage.IStreamedFileDataRequest.FailAndClose(Windows.Storage.StreamedFileFailureMode)
-api-type: winrt method
---

<!-- Method syntax
public void FailAndClose(Windows.Storage.StreamedFileFailureMode failureMode)
-->

# Windows.Storage.IStreamedFileDataRequest.FailAndClose

## -description
<!--{annotation author="miriamw" time="3/5/2012 6:10:33 PM"}TODO: Ask dfields: This is actually set by the app that is providing the stream, correct? So shouldn't we say "Sets ..." or soemthing like that?-->
Indicates that the data can't be streamed and releases system resources that are exposed by the current stream indicating that the data request is complete.

## -parameters
### -param failureMode
The enum value that indicates why the data stream can't be accessed.

## -remarks

## -examples

## -see-also
