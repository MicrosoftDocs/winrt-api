...
## -returns
When this method completes, it returns the number of bytes (type [UInt32](/dotnet/api/system.uint32?redirectedfrom=MSDN)) that were written to the stream. If the app specifies a function to monitor progress, that function receives the number of bytes (type [UInt32](/dotnet/api/system.uint32?redirectedfrom=MSDN)) written so far.
