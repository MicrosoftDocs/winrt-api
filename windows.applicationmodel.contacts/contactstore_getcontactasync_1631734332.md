---
-api-id: M:Windows.ApplicationModel.Contacts.ContactStore.GetContactAsync(System.String)
-api-type: winrt method
---

<!-- Method syntax
public Windows.Foundation.IAsyncOperation<Windows.ApplicationModel.Contacts.Contact> GetContactAsync(System.String contactId)
-->

# Windows.ApplicationModel.Contacts.ContactStore.GetContactAsync

## -description
Retrieves a [Contact](contact.md) object representing the contact with the specified [Id](contact_id.md) value.

## -parameters
### -param contactId
The [Id](contact_id.md) of the contact to be retrieved.

## -returns
An asynchronous operation that returns a [Contact](contact.md) object on successful completion. If you use [Asynchronous programming](http://msdn.microsoft.com/library/23fe28f1-89c5-4a17-a732-a722648f9c5e), the result type is a single [Contact](contact.md) object.

## -remarks

## -examples

## -see-also
[Contact](contact.md), [Asynchronous programming](http://msdn.microsoft.com/library/23fe28f1-89c5-4a17-a732-a722648f9c5e)
## -capabilities
contactsSystem
