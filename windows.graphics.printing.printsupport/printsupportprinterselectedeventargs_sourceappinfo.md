---
-api-id: P:Windows.Graphics.Printing.PrintSupport.PrintSupportPrinterSelectedEventArgs.SourceAppInfo
-api-type: winrt property
---

# Windows.Graphics.Printing.PrintSupport.PrintSupportPrinterSelectedEventArgs.SourceAppInfo

<!--
public Windows.ApplicationModel.AppInfo SourceAppInfo { get; }
-->


## -description

Gets an object that provides information about the source app associated with the [PrintSupportExtensionSession.PrinterSelected](/uwp/api/windows.graphics.printing.printsupport/printsupportextensionsession_printerselected) event. 

## -property-value

The app information object.

## -remarks

## -see-also

## -examples


