...
## -remarks
The [{Binding} markup extension](https://msdn.microsoft.com/library/3bafe7b5-af33-487f-9ad5-beafd65d04c3) enables you to specify a Binding value as a single attribute string in XAML, including setting Binding properties such as [Path](binding_path.md) and [Source](binding_source.md). For more info about data binding concepts, see [Data binding in depth](https://msdn.microsoft.com/library/41e1b4f1-6caf-4128-a61a-4e400b149011).

The Binding class might be considered the code-behind exposure of the [{Binding} markup extension](https://msdn.microsoft.com/library/3bafe7b5-af33-487f-9ad5-beafd65d04c3). If a binding is already applied to a target (which happens when the XAML is loaded), you can't set the read-write properties of a Binding object to change how a binding behaves at run-time. Any XAML-defined binding should be considered immutable. But you can create a new Binding object, set its properties, and establish a new binding on a specific UI element target using [FrameworkElement.SetBinding](../windows.ui.xaml/frameworkelement_setbinding_28713777.md). For more info, see [Creating bindings in code](https://msdn.microsoft.com/library/41e1b4f1-6caf-4128-a61a-4e400b149011).
...
For more info on XAML attribute usage for properties that can take a Binding, or that can otherwise be set to a data-bound value, see [{Binding} markup extension](https://msdn.microsoft.com/library/3bafe7b5-af33-487f-9ad5-beafd65d04c3).
