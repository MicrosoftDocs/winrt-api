...
> [!IMPORTANT]
> StoreContext APIs require a valid retail license issued by the Microsoft Store. A developer license is not a retail license and is not supported; using one may result in errors or crashes. Apps that declare the [appLicensing](https://learn.microsoft.com/windows/uwp/packaging/app-capability-declarations#restricted-capability-list) restricted capability may launch without a license present locally. When such an app calls [GetDefault](storecontext_getdefault_846721868.md), the system acquires a retail license automatically. This acquisition requires network connectivity and a valid entitlement. If acquisition fails (device offline, no entitlement), the app continues to run but StoreContext API calls that depend on a license will fail. Developers must understand that apps using **appLicensing** and StoreContext functionality may lose StoreContext functionality if license acquisition fails during StoreContext initialization.
